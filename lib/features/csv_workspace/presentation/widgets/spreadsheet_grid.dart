...
class _SpreadsheetGridState extends ConsumerState<SpreadsheetGrid> {
  ...
  void _scrollToSelectedCell(int listViewIndex) {
    if (!_scrollController.hasClients) return;
    
    final double itemOffset = listViewIndex * 32.0;
    final double viewportHeight = _scrollController.position.viewportDimension;
    final double currentScroll = _scrollController.offset;
    
    // If the item is below the viewport
  ...
  void _scrollToSelectedColumn(int colIndex) {
    if (!_horizontalScrollController.hasClients) return;
    
    final columnWidths = ref.read(columnWidthsProvider);
    double leftOffset = 50.0;
    for (int i = 0; i < colIndex; i++) {
      leftOffset += columnWidths[i] ?? 120.0;
    }
    final double colWidth = columnWidths[colIndex] ?? 120.0;
    final double rightOffset = leftOffset + colWidth;
    
    final double viewportWidth = _horizontalScrollController.position.viewportDimension;
    final double currentScroll = _horizontalScrollController.offset;
    
    // If the column's right edge is to the right of the viewport
    if (rightOffset > currentScroll + viewportWidth) {
      _horizontalScrollController.jumpTo(rightOffset - viewportWidth);
    }
    // If the column's left edge is to the left of the viewport (excluding the sticky index column at 50px)
    else if (leftOffset < currentScroll + 50.0) {
      _horizontalScrollController.jumpTo(leftOffset - 50.0);
    }
  }
